...
functionQueries.getDatosProducto = (req,res) =>{
    connection.tx(t=>{
        return t.one("SELECT * FROM public.producto p WHERE p.codigo =$1",req.params.id);
    })
    .then(data=>{
...
functionQueries.postProductos = (req,res,next) => {
    connection.tx(t=>{
        return t.none("INSERT INTO producto VALUES ($1, $2, $3, $4, $5, $6)", [req.body.codigo, req.body.nombre, req.body.precio, req.body.prioridad, req.body.tipo, req.body.descricion])
    })
    .then(data=>{
...
functionQueries.editProducto= (req,res)=>{
    ...
    connection.tx(t=>{
        return t.none('UPDATE public."producto" SET nombre = $1, tipo = $3, prioridad = $4, descripcion = $5 WHERE codigo = $2',[req.body.nombre,req.body.codigo,req.body.tipo,req.body.prioridad,req.body.descripcion])
    })
    .then(data=>{
        console.log('gasgasd');
